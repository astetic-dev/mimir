...
2. [seen] Its headings include "Voice rules" -> the content is the voice rules.
...
- Deeper step tested and rejected: "the builder did not know the guardrail".
  ...
  sit in this file.
- Shallower step passed over: the contract could have named a section of the
  style guide instead of the whole file (family 6). The routed guide is 22
  lines, so a section scope changes nothing.
...
CONTRIBUTING FACTORS: A target tree is the architect's question and I decline to
draw one. You should move the voice rules into a reference file and name that
file in the write stage's Inputs table.
