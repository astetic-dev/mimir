...
EVIDENCE CHAIN:
1. `CLAUDE.md` is 135 lines against an entry-file limit of 60 -> the entry file
   carries content.
2. Its headings include "Voice rules" -> the content is the voice rules.
3. No stage contract names that file -> the rules cannot be routed to a step.
...
WEAKEST LINK: Link 3 rests on the mined contract inputs being complete.
